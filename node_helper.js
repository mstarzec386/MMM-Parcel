...
module.exports = NodeHelper.create({
  start: function () {
    ...
    fs.readFile("modules/" + this.name + "/force_trans.json", "utf8", function (
      err,
      data
    ) {
      if (!err) {
        ...
        console.log(
          "Message " +
            this.name +
            ': no "force_trans.json" translation file found. This is no problem'
        );
      }
    });
  },
  ...
  translateMessage: function (orig, lang, mplace, i) {
    FREEtranslate(orig, { to: lang })
      .then((res) => {
        var trans = forceTrans[orig] || res.text;
        //			console.log("TRANSLATED: ", orig, 'into ', trans);
        parcelResult.trackings[mplace.p].checkpoints[
          mplace.cp
        ].translated_message = trans;
        lastTrans[i] = trans;
  ...
  translate: function (data, lang) {
    if (translationErrcount > 100) {
      return;
    }
    if (translationErrcount > 90) {
      console.log(
        Date(),
        "Too many translation API call errors, translations will be stopped (soon) ",
        100 - translationErrcount
      );
    }
    ...
    if (this.equalsarray(lastTexts, mstrings)) {
      for (i = 0; i < mplaces.length; i++) {
        parcelResult.trackings[mplaces[i].p].checkpoints[
          mplaces[i].cp
        ].translated_message = lastTrans[i];
      }
      return;
    }
  ...
  fetchShipments: function () {
    var self = this;
    var aftershipAPI = new aftershipSDK.AfterShip(this.config.apiKey);
    aftershipAPI.tracking
      .listTrackings()
      .then((data) => {
        parcelResult = data;
        ...
        if (self.config.debug) {
          console.log("RESULT.DATA-T = ", JSON.stringify(data));
        }
      })
      .catch((err) => {
  ...
  startUpdateNext: function () {
    ...
    setTimeout(function () {
      self.UpdateNext();
    }, 3500); // give the API some time to return before updating
  },
