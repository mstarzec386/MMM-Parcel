...
Module.register("MMM-Parcel", {
  ...
  suspend: function () {
    this.sendSocketNotification(
      "INTERVAL_SET",
      Math.max(900000, this.config.updateInterval * 2)
    );
  },
  ...
  getDom: function () {
    ...
    for (var i = 0; i < parcelList.length; i++) {
      if (
        !(
          (this.config.hideDelivered === true &&
            parcelList[i].tag == "Delivered") ||
          (parcelList[i].tag == "Delivered" &&
            Number.isInteger(this.config.hideDelivered) &&
            moment().diff(parcelList[i].updated_at, "days") >=
              this.config.hideDelivered)
        ) &&
    ...
    var isCompact =
      this.config.compactness == 1 || this.config.compactness == 2;
    ...
    if (this.config.isSorted) {
      l = l.sort(function (a, b) {
        return parcelStatus.indexOf(a.tag) - parcelStatus.indexOf(b.tag);
      });
    }
    ...
    for (let p of l) {
      ...
      if (count++ == this.config.maxNumber) {
        break;
      }
      ...
      var thisParcelIcon = this.makeParcelIconWrapper(
        parcelIcons[parcelStatus.indexOf(p.tag)],
        parcelIconColor[parcelStatus.indexOf(p.tag)]
      );
      var lastLoc =
      ...
      if (clockTime) {
        ...
        var today =
          delivery >= startofDay && delivery < startofDay + 24 * 60 * 60 * 1000;
        var thisweek =
          delivery >= startofDay + 24 * 60 * 60 * 1000 &&
          delivery < startofDay + 7 * 24 * 60 * 60 * 1000;
        if (timeFormat === 2) {
          if (clockTime.includes("T")) {
            deliverywrapper.innerHTML =
              (p.expected_delivery ? this.config.expectedDeliveryText : "") +
          ...
          if (today && clockTime.includes("T")) {
            deliverywrapper.innerHTML = moment(clockTime).format(
              config.timeFormat == 24 ? "HH:mm" : "ha"
            );
          } else if (thisweek) {
      ...
      if (timeFormat === 1) {
        ...
        headerwrapper.style.maxWidth =
          "calc(" + this.config.maxWidth + " - 120px)";
        ...
        parcelWrapperheaderline.appendChild(deliverywrapper);
        headerwrapper.style.maxWidth =
          "calc(" + this.config.maxWidth + " - 80px)";
      }
      ...
      if (clockTime && timeFormat === 2) {
        ...
        if (isNarrow && timeSlug) {
          deliverywrapper.innerHTML =
            deliverywrapper.innerHTML + " (" + timeSlug + ")";
        }
      ...
      if (p.checkpoints != undefined && p.checkpoints.length != 0) {
        ...
        // location icon
        parcelWrapperinfoline.appendChild(
          this.makeParcelIconWrapper("fa fa-location-arrow fa-fw")
        );
        ...
        var extraInfoText = "";
        var message = lastLoc.translated_message
          ? lastLoc.translated_message
          : lastLoc.message;
        var sepNeed = false;
        ...
        if (
          lastLoc &&
          lastLoc.message &&
          lastLoc.message.indexOf("to be collected") != -1 &&
          p.tag === "Delivered"
        ) {
          thisParcelIcon.style.color =
            parcelIconColor[parcelStatus.indexOf("OutForDelivery")];
        }
        parcelWrapperinfoline.appendChild(infotextwrapper);
        // add infoline unless very compact style
        if (!(isveryCompact || (isNarrow && isCompact))) {
      ...
      if (
        (clockTime && !isCompact) ||
        (clockTime && isNarrow && !isveryCompact)
      ) {
